    ...
    <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
      <div>
        <p class="text-sm text-gray-700">
          Showing
          <span class="font-medium" v-text="data.meta.from"></span>
          to
          <span class="font-medium" v-text="data.meta.to"></span>
          of
          <span class="font-medium" v-text="data.meta.total"></span>
          results
        </p>
      </div>
      <div>
        <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
          ...
          <template v-for="(link, i) in data.meta.links">
            <a @click.prevent="setPage(link.label)"
               v-if="i !== 0 && i !== data.meta.links.length - 1"
               :href="link.url"
               v-text="link.label"
